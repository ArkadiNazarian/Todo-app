...
export const View = (props: IFormModel) => (
    <Box sx={{ background: "#0000001a", width: "16rem", paddingLeft: 3, paddingRight: 3, paddingTop: 5, paddingBottom: 2, minHeight: "50vh", borderRadius: "0px 0px 6px 0px" }}>
        ...
        <Box onClick={() => props.goto_today()} sx={{ ':hover': { background: "#00000033", cursor: "pointer" }, background: props.on_today ? "#00000033" : "", fontWeight: 'light', padding: 1, marginBottom: 1, borderRadius: 1, height: 1, display: "flex", alignItems: "center", justifyContent: "space-between" }}><Typography sx={{ display: "flex", alignItems: "center" }}><TodayOutlinedIcon sx={{ color: "#692fc2", marginRight: "8px", fontSize: 20 }} />Today</Typography><Typography color="#dd4b39">{props.tasks_today_number}</Typography></Box>
        <Box onClick={() => props.goto_calendar()} sx={{ ':hover': { background: "#00000033", cursor: "pointer" }, background: props.on_calendar ? "#00000033" : "", fontWeight: 'light', padding: 1, marginBottom: 1, borderRadius: 1, height: 1, display: "flex", alignItems: "center", justifyContent: "space-between" }}><Typography sx={{ display: "flex", alignItems: "center" }}><CalendarMonthOutlinedIcon sx={{ color: "#ff9933", marginRight: "8px", fontSize: 20 }} />Calendar</Typography></Box>
        <Box onClick={() => props.goto_done()} sx={{ ':hover': { background: "#00000033", cursor: "pointer" }, background: props.on_done ? "#00000033" : "", fontWeight: 'light', padding: 1, marginBottom: 1, borderRadius: 1, height: 1, display: "flex", alignItems: "center", justifyContent: "space-between" }}><Typography sx={{ display: "flex", alignItems: "center" }}><CloudDoneOutlinedIcon sx={{ color: "#058527", marginRight: "8px", fontSize: 20 }} />Done</Typography><Typography color="#058527">{props.done_number}</Typography></Box>
                    ...
                    <Box key={index} onClick={() => props.handler_project(value.id)} fontSize={17} fontWeight={400} position="relative">
                        <Box sx={{ ':hover': { background: "#00000033", cursor: "pointer" }, padding: 1, borderRadius: 1, display: "flex", flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginBottom: 1, background: (props.project_id === value.id && props.on_project) ? "#00000033" : "" }}><Box sx={{ display: "flex", alignItems: "center" }}><Typography sx={{ width: "12px", height: "12px", background: value.color, borderRadius: 4, marginRight: 1 }}></Typography>{value.project_title}</Box><MoreHorizIcon onClick={(e) => props.project_menu(value.id, e)} /></Box>
                        {
                            (props.open_project_menu && props.selected_project_id === value.id) && <Paper sx={{ position: "absolute", right: -1, top: 30, padding: 1, zIndex: 1, boxShadow: "10px 3px 5px -1px rgb(0 0 0 / 20%),0px 6px 10px 0px rgb(0 0 0 / 14%), 0px 1px 18px 0px rgb(0 0 0 / 12%)" }}>
                                <Typography sx={{ ':hover': { background: "#00000099" }, cursor: "pointer", padding: 0.5, borderRadius: 1, display: "flex", alignItems: "center" }} onClick={(e) => props.edit_project.handler_open_edit_project_modal(value.id,e)}><EditOutlinedIcon />Edit</Typography>
                                <Typography color="#FF0000" sx={{ ':hover': { background: "#00000099" }, cursor: "pointer", padding: 0.5, borderRadius: 1, display: "flex", alignItems: "center" }} onClick={(e) => props.handler_delete_project(value.id,e)}><DeleteOutlineOutlinedIcon />Delete</Typography>
                            </Paper>
                        }
                    </Box>
                ))
